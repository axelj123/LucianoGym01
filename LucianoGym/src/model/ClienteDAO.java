...
public class ClienteDAO {
    ...
    public void ListarHistorialMembresias(JTable tblHistorial) {
        String sql = "SELECT * FROM vistaUsuariosHistorial;";
        try {
            con = cn.getConexion();
            ...
            while (rs.next()) {
                ...
                model.addRow(new Object[]{cl.getCodigo(), cl.getApellidos(), cl.getNombres()});
...
public List<ClienteHistorial> obtenerClientesDesdeBaseDeDatos(int idCliente) {
    String sql = "SELECT precioMembresia, MontoPagado, FechaInicio, FechaFin FROM historialsocio WHERE ClienteID = ? ORDER BY FechaInicio";

    try {
        con = cn.getConexion();
        ps = con.prepareStatement(sql);
        ps.setInt(1, idCliente); // Asignar el ID del cliente al parámetro de la consulta
        rs = ps.executeQuery();

        List<ClienteHistorial> clientes = new ArrayList<>();

        while (rs.next()) {
            // Obtener datos del resultado de la consulta
            double precioMembresia = rs.getDouble("precioMembresia");
            double montoPagado = rs.getDouble("MontoPagado");
            Date fechaInicio = rs.getDate("FechaInicio");
            Date fechaFin = rs.getDate("FechaFin");

            // Crear un objeto de la clase ClienteHistorial
            ClienteHistorial clienteHistorial = new ClienteHistorial(idCliente, precioMembresia, montoPagado, fechaInicio, fechaFin);

            // Agregar el objeto a la lista
            clientes.add(clienteHistorial);
        }
    ...
    public void ListarPagosDelHistorial(int codigo, JTextField Membresia, JTextField Pagado) {
        String sql = "CALL ObtenerDatosPorID(?)";
        try {
            con = cn.getConexion();
            ps = con.prepareStatement(sql);
            ps.setInt(1, codigo);
            ...
            while (rs.next()) {
                ...
                Membresia.setText(String.valueOf(cl.getPrecioMembresia()));
                Pagado.setText(String.valueOf(cl.getMontoPagado()));
    ...
    public void SumaMembresiasHistorial(int codigo, JLabel total) {
        String sql = "CALL ObtenerTotalMembresias(?)";
        try {
            con = cn.getConexion();
            ps = con.prepareStatement(sql);
            ps.setInt(1, codigo);
            ...
            while (rs.next()) {
                double totalMembresias = rs.getDouble("TotalMembresias");
                total.setText(String.valueOf(totalMembresias));
    ...
    public void ListarHistorialesCliente(int codigo, JTable tblHistorialcliente, JLabel total) {
        String sql = "{CALL ObtenerHistorialPorCliente(?)}";
        try {
            con = cn.getConexion();
            ps = con.prepareStatement(sql);
            ps.setInt(1, codigo);
            rs = ps.executeQuery();
            DefaultTableModel model = (DefaultTableModel) tblHistorialcliente.getModel();
            model.setRowCount(0);
            ...
            while (rs.next()) {
                ...
                // Agrega una fila con contador, FechaInicio y FechaFin
                model.addRow(new Object[]{id, cl.getFechaInicio(), cl.getFechaFin()});
            }
            int n = tblHistorialcliente.getRowCount();
    ...
    public void HistorialClienteCodigo(int codigo, JTextField txtDni, JTextField txtTelefono) {
        String sql = "CALL ObtenerDatosPorCodigo(?)";
        try {
            con = cn.getConexion();
            ps = con.prepareStatement(sql);
            ps.setInt(1, codigo);
