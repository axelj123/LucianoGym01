...
public class ClienteDAO {
    ...
    public void ListarClientesBuscar(JTable tblBuscar, JLabel cantidad) {
        String sql = "SELECT *\n"
                + "FROM Usuarios u";
        ...
        try {
            ...
            rs = ps.executeQuery();
            tblBuscar.setRowHeight(100); // Establece un alto fijo para las filas

            // Crear un CustomTableModel que incluye una columna para la imagen
            String[] columnNames = {"Codigo", "DNI", "Nombres", "Apellidos", "Telefono", "FechaNacimiento", "Foto"};
            ...
            while (rs.next()) {
                ...
                if (fotoBytes != null) {
                    ...
                    int altoDeseado = tblBuscar.getRowHeight(); // Obtener el alto de fila fijo

                    // Calcular el ancho proporcional manteniendo la relación de aspecto
                    int anchoProporcional = (imagen.getIconWidth() * altoDeseado) / imagen.getIconHeight();

                    // Escalar la imagen al ancho y alto deseados
                    Image imagenEscalada = imagen.getImage().getScaledInstance(anchoProporcional, altoDeseado, Image.SCALE_SMOOTH);

                    // Crear un nuevo ImageIcon a partir de la imagen escalada
                    ImageIcon imagenEscaladaIcon = new ImageIcon(imagenEscalada);

                    // Agregar los datos a una nueva fila en el modelo de la tabla
                    model.addRow(new Object[]{cliente.getCodigo(), cliente.getDni(), cliente.getNombres(), cliente.getApellidos(), cliente.getTelefono(), cliente.getFechaNacimiento(), imagenEscaladaIcon});
                } else {
                    // Si los datos de la imagen son nulos, agregar un valor nulo en lugar de un ImageIcon
                    model.addRow(new Object[]{cliente.getCodigo(), cliente.getDni(), cliente.getNombres(), cliente.getApellidos(), cliente.getTelefono(), cliente.getFechaNacimiento(), null});
                }

            }

            // Configurar el renderizador personalizado para la columna de imagen
            tblBuscar.getColumnModel().getColumn(6).setCellRenderer(new ImageRenderer());
...
public void buscarClientes(Cliente cl, JTable tblBuscar, JLabel lblTotal) {
    ...
    public void ListaPagosPendientes(JTable mostrar, JLabel lblPagosPendientes) {
        ...
        String sql = "SELECT u.Codigo, u.Apellidos, u.Nombres,p.PrecioMembresia,p.MontoPagado,p.MontoRestante\n"
                + "FROM Usuarios u\n"
                + "join PagoMembresias p on u.Codigo=p.ClienteID\n"
                + "where p.MontoRestante>0";
        conexion cn = new conexion();
        Connection con;
        PreparedStatement ps;
        ResultSet rs;
