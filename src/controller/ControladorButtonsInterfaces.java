...
public class ControladorButtonsInterfaces implements ActionListener, MouseListener, KeyListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (sourceButton == np.btnVenderProducto) {
            np.jTabbedPane1.setSelectedIndex(9);
            np.tblComprasUnit.fixTable(np.jScrollPane13);
            np.tblComprasUnit.addMouseListener(this);
            np.tblComprasPor.fixTable(np.jScrollPane12);
            np.tblComprasPor.addMouseListener(this);
            prdao.listarProductosUnitaros(np.tblComprasUnit, np.lblTotal);
            prdao.listarProductosPorcion(np.tblComprasPor, np.lblTotal);
        }
...
// Método auxiliar para configurar la validación de longitud mínima de texto
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        if (sourcetable == np.tblComprasUnit) {

            if (e.getButton() == MouseEvent.BUTTON1 && e.getClickCount() == 1) {
                np.txtCodVentaPro.setText("");
                int fila = np.tblComprasUnit.rowAtPoint(e.getPoint());
                int columna = np.tblComprasUnit.columnAtPoint(e.getPoint());
                if ((fila > -1) && (columna > -1)) {
                    ProductoUnitario pr = new ProductoUnitario();
                    pr.setProductoID(Integer.parseInt(np.tblComprasUnit.getValueAt(fila, 0).toString()));
                    pr.setNombre(np.tblComprasUnit.getValueAt(fila, 1).toString());
                    pr.setPrecio(Double.parseDouble(np.tblComprasUnit.getValueAt(fila, 4).toString()));

                        int stock = (Integer.parseInt(np.tblComprasUnit.getValueAt(fila, 5).toString()));

                    if (stock > 0) {

                        np.txtCodVentaPro.setText(String.valueOf(pr.getProductoID()));
                        np.txtNombreVentaPro.setText(pr.getNombre());
                        np.txtPrecioProVen.setText(String.valueOf(pr.getPrecio()));
                        prdao.mostrarFotoProducto(np.lblFotoProVent, pr.getProductoID());
                        np.txtCantidadVentaPro.addKeyListener(new KeyAdapter() {
                            @Override
                            public void keyReleased(KeyEvent e) {
                                // Obtén la cantidad ingresada por el usuario
                                try {
                                    int cantidad = Integer.parseInt(np.txtCantidadVentaPro.getText());
                                    double precio = Double.parseDouble(np.txtPrecioProVen.getText());
                                    ...
                                    if (cantidad > stock) {
                                        ...
                                        np.txtCantidadVentaPro.setText("");
                                    } else {
                                        // Realiza el cálculo basado en la cantidad ingresada y el precio
                                        double total = cantidad * precio;
                                        np.txtTotalVenta.setText(String.valueOf(total));
                                    }
                                } catch (NumberFormatException ex) {
                                    // Manejar la excepción si la entrada no es un número válido
                                    np.txtTotalVenta.setText(""); // Puedes establecer un valor predeterminado o mostrar un mensaje de error
                                }
                            }
                        });
                    }
        ...
        if (sourcetable == np.tblComprasPor) {
            if (e.getButton() == MouseEvent.BUTTON1 && e.getClickCount() == 1) {
                np.txtCodVentaPro.setText("");
                int fila = np.tblComprasPor.rowAtPoint(e.getPoint());
                int columna = np.tblComprasPor.columnAtPoint(e.getPoint());
                if ((fila > -1) && (columna > -1)) {
                    ProductoUnitario pr = new ProductoUnitario();
                    pr.setProductoID(Integer.parseInt(np.tblComprasPor.getValueAt(fila, 0).toString()));
                    pr.setNombre(np.tblComprasPor.getValueAt(fila, 1).toString());
                    pr.setPrecio(Double.parseDouble(np.tblComprasPor.getValueAt(fila, 4).toString()));
                    int stock = (Integer.parseInt(np.tblComprasPor.getValueAt(fila, 6).toString()));
